...
class Qwen2MLP(nn.Module):
    def __init__(self, config, layer_idx, sparsity_ratio=0.5, sparse_mode="dense"):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
    ...
    def forward(self, hidden_state):
        ...
        if sparse_mode == "static":
            ...
            # print(f"self.gate_proj: {self.gate_proj}")
            # print(f"self.up_proj: {self.up_proj}")
            # print(f"act: {act.size()}")
            # print(f"up_out: {up_out.size()}")
            down_input = act * up_out
            # print(f"self.down_proj: {self.down_proj}")
            # print(f"down_input: {down_input.size()}")
            hidden = self.down_proj(down_input)
            if self.sparse_mode == "prune":
                # pass
            
                ##### intermeidiate_size
                ### tokens, intermediate_size x intermeidiate_size, hidden_size --> intermediate_size

                # print(f"self.down_proj: {self.down_proj}")
                # print(f"down_input: {down_input.size()}")
                W_mean = self.down_proj.weight.data.mean(0)
                # print(f"W_mean: {W_mean.size()}")
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2_5_VLTextConfig, layer_idx: int, record: bool = False):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not self.skip_mode in ["block", "attn"]:
            ...
            # print(f"Layer {self.layer_idx}, skip_mode: {self.skip_mode}")
