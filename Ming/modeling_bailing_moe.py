...
class BailingMoeMLP(nn.Module):
    def __init__(self, config: BailingMoeConfig, intermediate_size: int, layer_idx: int, sparsity_ratio=0.5, sparse_mode="dense"):
        super().__init__()
        self.config = config
    ...
    def forward(self, hidden_state):
        ...
        if sparse_mode == "static":
            # 静态 mask：只保留前 k% 个 channel
            k = int(self.intermediate_size * self.sparsity_ratio)
            act = act[..., :k]
            up_out = up_out[..., :k]
            down_input = act * up_out

            down_proj_weight = self.down_proj.weight[:, :k]  # shape [hidden, k]
            hidden = F.linear(down_input, down_proj_weight, bias=None)

        elif sparse_mode == "random":
            B, D = act.shape
            k = int(D * self.sparsity_ratio)
            mask = torch.zeros_like(act).bernoulli(self.sparsity_ratio)
            # print(f"mask: {mask.mean()}")
            down_input = act * up_out * mask
            hidden = self.down_proj(down_input)

        elif sparse_mode == "topk":
            k = int(act.shape[-1] * self.sparsity_ratio)
            topk_vals, topk_idx = torch.topk(act, k, dim=-1)
            mask = torch.zeros_like(act).scatter(-1, topk_idx, 1.0)
            down_input = act * up_out * mask
            hidden = self.down_proj(down_input)

        elif sparse_mode == "mask": 
            down_input = act * up_out 
            down_input = down_input * self.mask.to(act.dtype)   # broadcast (B,T,inter)
            hidden = self.down_proj(down_input)
            ...
            if self.sparse_mode == "prune":
                ...
                # self.act_sum += down_input.abs().mean(dim=(0, 1)).cpu()
                self.act_sum = (down_input * W_mean).abs().mean(dim=(0, 1)).cpu() if self.act_sum is None else self.act_sum + (down_input * W_mean).abs().mean(dim=(0, 1)).cpu()
                self.act_cnt += 1
                # print(f"self.act_sum: {self.act_sum} | self.act_cnt: {self.act_cnt}")
    ...
    def __init__(self, config: BailingMoeConfig, layer_idx: int):
        ...
        if config.num_shared_experts is not None:
            self.shared_experts = BailingMoeMLP(
                config=config, intermediate_size=config.moe_intermediate_size * config.num_shared_experts, layer_idx=layer_idx, 
            )

    def _setup_experts(self):
        self.experts = nn.ModuleList(
            [
                BailingMoeMLP(config=self.config, intermediate_size=self.config.moe_intermediate_size, layer_idx=self.layer_idx)
...
class BailingMoeAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(value_states.dtype) ### query_states.dtype

        attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        # print(f"attn_weights: {attn_weights.dtype}, value_states {value_states.dtype}")
...
class BailingMoeDecoderLayer(nn.Module):
    def __init__(self, config: BailingMoeConfig, layer_idx: int, record=False):
        super().__init__()
        self.layer_idx = layer_idx
        self.hidden_size = config.hidden_size
        ...
        self.mlp = (
            BailingMoeSparseMoeBlock(config, self.layer_idx)
            if (config.num_experts is not None and layer_idx >= config.first_k_dense_replace)
            else BailingMoeMLP(config=config, intermediate_size=config.intermediate_size, layer_idx=self.layer_idx)
        )
