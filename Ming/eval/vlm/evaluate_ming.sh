...
if [ ${DATASET} == "mmbench-dev-en" ]; then
    torchrun \
      ...
      -m eval.vlm.eval.mmbench.evaluate_mmbench_ming --datasets mmbench_dev_20230712 \
                                                --keep_ratio $keep_ratio \
                                                --calibration_samples $calibration_samples \
                                                --compressed_layers_und $compressed_layers_und \
                                                --sparse_mode $sparse_mode \

      output_dir=/mnt/bn/seed-aws-va/shwai.he/cdt-hf/MMBench/Ming/$compressed_layers_und/$sparse_mode/sparsity_${keep_ratio}/samples_${calibration_samples}
...
if [ ${DATASET} == "mmvp" ]; then
    ...
    # python -m eval.vlm.eval.mmvp.evaluate_mmvp_ming --datasets MMVP "${ARGS[@]:1}"
fi
