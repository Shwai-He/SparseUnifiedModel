...
class PackedAttentionMoT(Qwen2Attention):
    def __init__(self, config, layer_idx: Optional[int] = None, sparse_mode="dense"):
        super().__init__(config, layer_idx)
        ...
        if self.sparse_mode == "prune":
            self.register_buffer("act_sum", torch.zeros(self.num_heads))     # ∑|h|
            self.register_buffer("act_cnt", torch.tensor(0, dtype=torch.long))       # batch 计数
    ...
    def forward_inference(
        ...
        packed_attn_output = flash_attn_varlen_func( # squence length, head, head_dim
            q=packed_query_states,
            k=merged_key_states,
            v=merged_value_states,
            cu_seqlens_q=cu_seqlens_q.to(torch.int32),
        ...
        if self.sparse_mode == "prune":
            if mode == "und":
                W_und = self.o_proj.weight.view(self.hidden_size, self.num_heads, self.head_dim).transpose(0, 1)    # (H, Dh, Dv)
                head_out = torch.einsum("shv,hdv->hsd", packed_attn_output, W_und)
            else: 
                W_und = self.o_proj.weight.view(self.hidden_size, self.num_heads, self.head_dim).transpose(0, 1)
                W_gen = self.o_proj_moe_gen.weight.view(self.hidden_size, self.num_heads, self.head_dim).transpose(0, 1)

                head_out_und = torch.einsum("shv,hdv->hsd", packed_attn_output[packed_text_indexes], W_und)
                head_out_gen = torch.einsum("shv,hdv->hsd", packed_attn_output[packed_vae_token_indexes], W_gen)
                head_out = torch.cat((head_out_und, head_out_gen), dim=1)

            act_sum = head_out.abs().mean(dim=(-2, -1)).cpu()          # (H,)
...
class Qwen2MoTDecoderLayer(nn.Module):
    def __init__(
        ...
        self.transformed_und = config.transformed_und if hasattr(config, "transformed_und") else False  
        self.transformed_gen = config.transformed_gen if hasattr(config, "transformed_gen") else False  
        ...
        config.shared_expert_intermediate_size = config.intermediate_size // (self.num_experts + self.num_shared_experts) * self.num_shared_experts if self.num_shared_experts is not None else None       
        remaining = config.intermediate_size - config.shared_expert_intermediate_size if config.intermediate_size is not None and config.shared_expert_intermediate_size is not None else None
        moe_inter = remaining // self.num_experts if self.num_experts is not None else None
        config.moe_intermediate_size = moe_inter

        if isinstance(self.transformed_und, bool):
            if not self.transformed_und:
                self.mlp = Qwen2MLP_Sparse(config, layer_idx, mode="und")
            else:
                self.mlp = Qwen2MoeSparseMoeBlock(config, mode="und")
        elif isinstance(self.transformed_und, list):
            if self.layer_idx not in self.transformed_und:        
                self.mlp = Qwen2MLP_Sparse(config, layer_idx, mode="und")
            else: 
                self.mlp = Qwen2MoeSparseMoeBlock(config, mode="und")
        else: 
            raise NotImplementedError("transformed_und must be a bool or a list of int.")

        if isinstance(self.transformed_gen, bool):
            if not self.transformed_gen:
                self.mlp_moe_gen = Qwen2MLP_Sparse(config, layer_idx, mode="gen")
            else:
                self.mlp_moe_gen = Qwen2MoeSparseMoeBlock(config, mode="gen")
        elif isinstance(self.transformed_gen, list):
            if self.layer_idx not in self.transformed_gen:
                self.mlp_moe_gen = Qwen2MLP_Sparse(config, layer_idx, mode="gen")
            else: 
                self.mlp_moe_gen = Qwen2MoeSparseMoeBlock(config, mode="gen")
        else:
            raise NotImplementedError("transformed_gen must be a bool or a list of int.")
    ...
    def convert_dense_to_sparse_moe_dual(
            ...
            shared_idx: Optional[torch.Tensor] = None,
        ):
        """
        Convert a dense `Qwen2MLP` into a `Qwen2MoeSparseMoeBlock`.
        """
        if mode == "und":
            self.mlp = self.dense_to_moe_with_explicit_shared(
                                                                self.mlp, 
                                                                mode, 
            ...
            self.mlp_moe_gen = self.dense_to_moe_with_explicit_shared(
                                                                self.mlp_moe_gen, 
                                                                mode, 
                                                                importance_scores=importance_scores, 
                                                                shared_ratio=shared_ratio, 
    ...
    def dense_to_moe_with_explicit_shared(
        ...
        shared_num = int(dense_inter * shared_ratio)
        shared_idx = sorted_idx[:shared_num]

        remaining_idx = sorted_idx[shared_num:]
        num_remaining = remaining_idx.numel()

        assert len(remaining_idx) % num_experts == 0
        chunks = remaining_idx.view(-1, num_experts)  
        chunks[1::2] = torch.flip(chunks[1::2], dims=[1])
        expert_indices = [chunks[:, i].contiguous().view(-1) for i in range(num_experts)]

        cfg = copy.deepcopy(self.config)
        cfg.num_experts                     = num_experts
        cfg.num_experts_per_tok             = top_k
        cfg.shared_expert_intermediate_size = shared_idx.numel()
        cfg.moe_intermediate_size           = num_remaining // num_experts

        moe_block = Qwen2MoeSparseMoeBlock(cfg, mode).to(device=device, dtype=dtype)
        ...
        with torch.no_grad():
            ...
            if hasattr(moe_block, "gate_img"):
                nn.init.zeros_(moe_block.gate_img.weight)

            if hasattr(moe_block, "gate_shared"):
                nn.init.zeros_(moe_block.gate_shared.weight)
    ...
    def forward_train(
        ...
        packed_sequence_[packed_und_token_indexes], routing_logits_und = self.mlp(
        ...
        packed_sequence_[packed_gen_token_indexes], routing_logits_gen = self.mlp_moe_gen(
            self.post_attention_layernorm_moe_gen(packed_sequence[packed_gen_token_indexes])
        )
        packed_sequence = residual + packed_sequence_
        if routing_logits_und is not None and routing_logits_gen is not None:
            routing_logits = torch.cat((routing_logits_und, routing_logits_gen), dim=0)

        return packed_sequence, routing_logits
    ...
    def forward_inference(
        ...
        if mode == "und":
            ...
            packed_query_sequence, routing_logits_und = self.mlp(packed_query_sequence, 
                                                                packed_text_indexes=packed_text_indexes,
                                                                packed_vit_token_indexes=packed_vit_token_indexes,
                                                                )
            routing_logits = routing_logits_und
        elif mode == "gen":
            packed_text_query_sequence = packed_query_sequence[packed_text_indexes]
            packed_vae_query_sequence = packed_query_sequence[packed_vae_token_indexes]
            ...
            packed_query_sequence_[packed_text_indexes], routing_logits_und = self.mlp(packed_text_query_sequence)
            packed_query_sequence_[packed_vae_token_indexes], routing_logits_gen = self.mlp_moe_gen(packed_vae_query_sequence)
            packed_query_sequence = packed_query_sequence_
            if routing_logits_und is not None and routing_logits_gen is not None:
...
#         packed_sequence_und, routing_logits_und = self.mlp(packed_sequence[packed_und_token_indexes])
#         packed_sequence_gen, routing_logits_gen = self.mlp_moe_gen(packed_sequence[packed_gen_token_indexes])
...
#         if routing_logits_und is not None and routing_logits_gen is not None: 
#             routing_logits = routing_logits_und + routing_logits_gen
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward_train(
        ...
        for decoder_layer in self.layers:
            packed_sequence, router_logits = decoder_layer(
                packed_sequence=packed_sequence,
                sample_lens=sample_lens,
                attention_mask=attention_mask,
                packed_position_embeddings=packed_position_embeddings,
    ...
    def forward_inference(
        ...
        # if packed_query_position_ids.device != packed_query_sequence.device: 
        #     packed_query_position_ids = packed_query_position_ids.to(packed_query_sequence.device)

        # print(f"packed_query_sequence: {packed_query_sequence.device}")
        # print(f"packed_query_position_ids: {packed_query_position_ids.device}")
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            # print(extra_inputs.keys())
            # print("model", packed_text_indexes is None, packed_vit_token_indexes is None)


            packed_query_sequence, past_key_values, router_logits = decoder_layer(
                packed_query_sequence=packed_query_sequence,
                query_lens=query_lens,
                packed_query_position_embeddings=packed_query_position_embeddings,
                packed_query_indexes=packed_query_indexes,
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def forward_train(
        ...
        # print("CausalLM", packed_text_indexes is None, packed_vit_token_indexes is None)
