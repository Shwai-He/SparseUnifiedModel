...
class Bagel(PreTrainedModel):
    ...
    def forward(
        ...
        # print(f"packed_text_indexes: {packed_text_indexes is None}, packed_vit_token_indexes: {packed_vit_token_indexes is None}")
        extra_inputs = {}
        if self.use_moe:
    ...
    def forward_cache_update_text(
        ...
        w_device     = embed_tokens.weight.device      # 权重所在设备
        ids_device   = packed_text_ids.device          # 当前 ids 设备

        # # 仅当设备不一致时才迁移，避免多余的 copy
        if ids_device != w_device:
            packed_text_ids = packed_text_ids.to(w_device)
    ...
    def forward_cache_update_vit(
        ...
        # print(f"packed_vit_token_indexes: {packed_vit_token_indexes}")

        if self.use_moe:
            extra_inputs = {"mode": "und"}

        extra_inputs["packed_text_indexes"] = packed_text_indexes
        extra_inputs["packed_vit_token_indexes"] = packed_vit_token_indexes
    ...
    @torch.no_grad
    def forward_cache_update_vae(   #### quite confused
        self,
        vae_model,
        past_key_values: NaiveCache,
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            if i == 0: 
                for layer in self.language_model.model.layers:
                    ...
                    setattr(layer.mlp, "mask", None)
                    setattr(layer.mlp_moe_gen, "mask", None)
            ...
            if i == 0:  #### prune parameters for next steps 
                ...
                for layer in self.language_model.model.layers:
                    ...
                    self.prune(layer.mlp_moe_gen, keep_ratio=keep_ratio, compressed_layers=compressed_layers_gen, )

            # print(f"i: {i}")
            # print(f"compressed_layers_und: {compressed_layers_und}")
            # print(f"compressed_layers_gen: {compressed_layers_gen}")
            # exit()
    ...
    def generate_text(
        ...
        while step < max_length:
            ...
            if self.use_moe:
                extra_inputs = {"mode": "und"}
                # extra_inputs = {"mode": "und", "gen_text": True}
    ...
    @torch.no_grad()
    def prune(self, layer, keep_ratio: float = 0.5, compressed_layers: list = [], ):
        ...
        if layer.layer_idx in compressed_layers:
            ...
            # print(f"{layer.layer_idx} keep: {mask.mean()}, keep_ratio: {keep_ratio}")
            if hasattr(layer, "mask"):
                del layer.mask
            layer.register_buffer("mask", mask)          # 自动随模型搬迁
            layer.sparse_mode = "mask"
            del layer.act_sum
            del layer.act_cnt
