...
# DATASET="mmvp"
DATASETs=("mmmu-val" "mmvet" "mmvp" "mmbench-dev-en" "mme")
DATASETs=("mmvp")
        ...
        for calibration_samples in "${Samples[@]}"; do
            ...
            # Parse options
                ...
                if [ ${DATASET} == "mmbench-dev-en" ]; then
                    ...
                    # -m eval.vlm.eval.mmbench.evaluate_mmbench --datasets $datasets \
                    #                                             --keep_ratio $keep_ratio \
                    #                                             --calibration_samples $calibration_samples \
                    #                                             --compressed_layers_und $compressed_layers_und \
                    #                                             --sparse_mode $sparse_mode \

                    output_dir=/mnt/bn/seed-aws-va/shwai.he/cdt-hf/results/understanding/MMBench/mmbench_dev_20230712/14-28/prune_0.0/samples_1
                    # output_dir=/mnt/bn/seed-aws-va/shwai.he/cdt-hf/results/understanding/MMBench/$datasets/$compressed_layers_und/${sparse_mode}_${keep_ratio}/samples_${calibration_samples}
                ...
                if [ ${DATASET} == "mmvp" ]; then
                    ...
                    --nproc_per_node=${GPUS} \
                    --master_port=${MASTER_PORT} \
                    -m eval.vlm.eval.mmvp.evaluate_mmvp --datasets MMVP \
                                                --keep_ratio $keep_ratio \
                                                --calibration_samples $calibration_samples \
                                                --compressed_layers_und $compressed_layers_und \
                                                --sparse_mode $sparse_mode
