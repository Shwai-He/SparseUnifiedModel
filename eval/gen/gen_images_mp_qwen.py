...
def fewshot_compress(keep_ratio, compressed_layers_und, compressed_layers_gen, calibration_samples, sparse_mode="prune", scores=None, record=False, ): 
    ...
    if sparse_mode == "random": 
        ...
        for i, layer in enumerate(pipe.text_encoder.model.language_model.layers):
            score = scores[i]
            k_und = prune(layer.mlp, keep_ratio=keep_ratio, compressed_layers=compressed_layers_und, scores=score)
            keep_und.append(k_und)
...
if __name__ == "__main__":
    ...
    if args.keep_ratio < 1.0 and args.strategy == "weight":
        ...
        fewshot_compress(args.keep_ratio, compressed_layers_und, compressed_layers_gen, calibration_samples, sparse_mode, scores)

    print(pipe.text_encoder.model.language_model)
